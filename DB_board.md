...
## 8. 공동구매 주문 테이블 (EVENT_ORDER1)

### 테이블 생성
```sql
-- 시퀀스 생성
CREATE SEQUENCE SEQ_EVENT_ORDER1
START WITH 1
INCREMENT BY 1
NOCACHE
NOCYCLE;
...
-- 공동구매 주문 테이블 생성
CREATE TABLE EVENT_ORDER1 (
    ORDER_SEQ NUMBER PRIMARY KEY,                -- 주문 번호 (PK, 시퀀스)
    PRODUCT_SEQ NUMBER NOT NULL,                 -- 상품 번호 (FK -> EVENT_PRODUCT1.PRODUCT_SEQ)
    MEMBER_ID VARCHAR2(50) NOT NULL,              -- 회원 ID (FK -> MEMBER1.ID)
    ORDER_QUANTITY NUMBER DEFAULT 1,              -- 주문 수량
    ORDER_PRICE NUMBER DEFAULT 0,                 -- 주문 가격
    ORDER_STATUS VARCHAR2(20) DEFAULT '주문완료', -- 주문 상태 (주문완료/취소/배송중/배송완료)
    DELIVERY_ADDRESS VARCHAR2(500),               -- 배송 주소
    DELIVERY_PHONE VARCHAR2(50),                  -- 배송 연락처
    CREATED_DATE DATE DEFAULT SYSDATE,            -- 주문일
    UPDATED_DATE DATE,                            -- 수정일
    CONSTRAINT FK_EVENT_ORDER1_PRODUCT FOREIGN KEY (PRODUCT_SEQ) REFERENCES EVENT_PRODUCT1(PRODUCT_SEQ),
    CONSTRAINT FK_EVENT_ORDER1_MEMBER FOREIGN KEY (MEMBER_ID) REFERENCES MEMBER1(ID)
);

-- 인덱스 생성
CREATE INDEX IDX_EVENT_ORDER1_SEQ ON EVENT_ORDER1(ORDER_SEQ);
CREATE INDEX IDX_EVENT_ORDER1_PRODUCT ON EVENT_ORDER1(PRODUCT_SEQ);
CREATE INDEX IDX_EVENT_ORDER1_MEMBER ON EVENT_ORDER1(MEMBER_ID);
CREATE INDEX IDX_EVENT_ORDER1_STATUS ON EVENT_ORDER1(ORDER_STATUS);
CREATE INDEX IDX_EVENT_ORDER1_CREATED ON EVENT_ORDER1(CREATED_DATE DESC);
```
...
VALUES (SEQ_EVENT_ORDER1.NEXTVAL, 1, 'hong123', 2, 1960000, '주문완료', '서울시 강남구 테헤란로 123', '010-1234-5678', SYSDATE);

INSERT INTO EVENT_ORDER1 (ORDER_SEQ, PRODUCT_SEQ, MEMBER_ID, ORDER_QUANTITY, ORDER_PRICE, ORDER_STATUS, DELIVERY_ADDRESS, DELIVERY_PHONE, CREATED_DATE)
VALUES (SEQ_EVENT_ORDER1.NEXTVAL, 1, 'kim456', 1, 980000, '배송중', '서울시 서초구 서초대로 456', '010-2345-6789', SYSDATE);
```

---

## 9. 비속어 필터 테이블 (PROFANITY_FILTER1)
...
## 10. 게시판별 공지사항 설정 테이블 (BOARD_NOTICE_SETTING1)
...
INSERT INTO BOARD_NOTIFICATION1 (NOTIFICATION_SEQ, MEMBER_ID, NOTIFICATION_TYPE, RELATED_POST_SEQ, RELATED_COMMENT_SEQ, NOTIFICATION_CONTENT, IS_READ, CREATED_DATE)
VALUES (SEQ_BOARD_NOTIFICATION1.NEXTVAL, 'kim456', '게시글댓글', 2, 2, '새로운 댓글이 작성되었습니다.', 'Y', SYSDATE - 1);
```
...
-- 공동구매 주문 등록
INSERT INTO EVENT_ORDER1 (ORDER_SEQ, PRODUCT_SEQ, MEMBER_ID, ORDER_QUANTITY, ORDER_PRICE, ORDER_STATUS, DELIVERY_ADDRESS, DELIVERY_PHONE, CREATED_DATE)
VALUES (SEQ_EVENT_ORDER1.NEXTVAL, ?, ?, ?, ?, '주문완료', ?, ?, SYSDATE);

-- 상품별 주문 목록 조회
SELECT * FROM EVENT_ORDER1 
WHERE PRODUCT_SEQ = ? 
ORDER BY CREATED_DATE DESC;
...
-- 주문 상태 업데이트
UPDATE EVENT_ORDER1 
SET ORDER_STATUS = ?, UPDATED_DATE = SYSDATE
WHERE ORDER_SEQ = ?;

-- 주문 취소
UPDATE EVENT_ORDER1 
SET ORDER_STATUS = '취소', UPDATED_DATE = SYSDATE
WHERE ORDER_SEQ = ? AND ORDER_STATUS = '주문완료';

-- 주문 취소 시 재고 복구
UPDATE EVENT_PRODUCT1 
SET STOCK_QUANTITY = STOCK_QUANTITY + (SELECT ORDER_QUANTITY FROM EVENT_ORDER1 WHERE ORDER_SEQ = ?),
    SOLD_QUANTITY = SOLD_QUANTITY - (SELECT ORDER_QUANTITY FROM EVENT_ORDER1 WHERE ORDER_SEQ = ?),
    UPDATED_DATE = SYSDATE
WHERE PRODUCT_SEQ = (SELECT PRODUCT_SEQ FROM EVENT_ORDER1 WHERE ORDER_SEQ = ?);
```
...
-- 공동구매 회원별 구매 현황 (공구이벤트 회원관리)
SELECT 
    O.MEMBER_ID,
    M.NAME AS MEMBER_NAME,
    M.NICKNAME AS MEMBER_NICKNAME,
    COUNT(O.ORDER_SEQ) AS ORDER_COUNT,
    SUM(O.ORDER_QUANTITY) AS TOTAL_QUANTITY,
    SUM(O.ORDER_PRICE) AS TOTAL_AMOUNT
FROM EVENT_ORDER1 O
LEFT JOIN MEMBER1 M ON O.MEMBER_ID = M.ID
WHERE O.ORDER_STATUS != '취소'
GROUP BY O.MEMBER_ID, M.NAME, M.NICKNAME
...
FROM EVENT_ORDER1 O
LEFT JOIN MEMBER1 M ON O.MEMBER_ID = M.ID
WHERE O.PRODUCT_SEQ = ? AND O.ORDER_STATUS != '취소'
...
FROM EVENT_ORDER1 O
LEFT JOIN MEMBER1 M ON O.MEMBER_ID = M.ID
LEFT JOIN EVENT_PRODUCT1 P ON O.PRODUCT_SEQ = P.PRODUCT_SEQ
WHERE O.MEMBER_ID = ? AND O.ORDER_STATUS != '취소'
ORDER BY O.CREATED_DATE DESC;
```
...
-- 공동구매 주문 시 재고 자동 차감 트리거
CREATE OR REPLACE TRIGGER TRG_EVENT_ORDER1_STOCK_DECREASE
AFTER INSERT ON EVENT_ORDER1
FOR EACH ROW
WHEN (NEW.ORDER_STATUS = '주문완료')
BEGIN
    -- 주문 완료 시 재고 차감 및 판매 수량 증가
    UPDATE EVENT_PRODUCT1 
    SET STOCK_QUANTITY = STOCK_QUANTITY - :NEW.ORDER_QUANTITY,
        SOLD_QUANTITY = SOLD_QUANTITY + :NEW.ORDER_QUANTITY,
        UPDATED_DATE = SYSDATE
    WHERE PRODUCT_SEQ = :NEW.PRODUCT_SEQ;
END;
/

-- 공동구매 주문 취소 시 재고 자동 복구 트리거
CREATE OR REPLACE TRIGGER TRG_EVENT_ORDER1_STOCK_RESTORE
AFTER UPDATE ON EVENT_ORDER1
FOR EACH ROW
WHEN (OLD.ORDER_STATUS = '주문완료' AND NEW.ORDER_STATUS = '취소')
BEGIN
    -- 주문 취소 시 재고 복구 및 판매 수량 감소
    UPDATE EVENT_PRODUCT1 
    SET STOCK_QUANTITY = STOCK_QUANTITY + :OLD.ORDER_QUANTITY,
        SOLD_QUANTITY = SOLD_QUANTITY - :OLD.ORDER_QUANTITY,
        UPDATED_DATE = SYSDATE
    WHERE PRODUCT_SEQ = :OLD.PRODUCT_SEQ;
END;
/
```
...
-- 공동구매 주문 상세 정보 뷰 (회원 정보 및 상품 정보 포함)
CREATE OR REPLACE VIEW V_EVENT_ORDER1_DETAIL AS
SELECT 
    O.ORDER_SEQ,
    O.PRODUCT_SEQ,
    O.MEMBER_ID,
    ...
    M.NAME AS MEMBER_NAME,
    M.NICKNAME AS MEMBER_NICKNAME,
    M.TEL1 || '-' || M.TEL2 || '-' || M.TEL3 AS MEMBER_PHONE,
    P.PRODUCT_NAME,
...
3. **트랜잭션 처리**: 재고 차감 시 동시성 제어가 필요합니다. 주문 등록과 재고 차감은 반드시 같은 트랜잭션 내에서 처리해야 합니다.
...
7. **주문 취소 처리**: 주문 취소 시 재고 복구와 판매 수량 감소를 반드시 함께 처리해야 합니다. 트리거를 사용하면 자동으로 처리됩니다.
8. **공동구매 회원관리**: EVENT_ORDER1 테이블을 통해 공동구매한 회원의 정보, 주문 수량, 주문 상태 등을 관리합니다.
9. **주문 상태 관리**: 주문 상태는 '주문완료', '취소', '배송중', '배송완료'로 관리하며, 취소는 '주문완료' 상태에서만 가능하도록 제한해야 합니다.
